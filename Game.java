...
class Game
{
	...
	Dealer dealer;
	String m, deckCount, bet, option, option2;
	...
	public Game(int playerCount) {
		...
		try{
			System.out.println("請輸入每位玩家籌碼 : ");
			m = s.next();
			if(Integer.parseInt(m) < 1){
				System.out.println("籌碼須大於等於1!");
				isError = true;
			}
			else{
	...
	public void battle(){
		if(!isOver && !isFinish){
			...
			try{
				for(int i = 0; i < players.size(); i++){
					System.out.println("玩家" + players.get(i).name + "請選擇 1)hit 2)stay : ");
					option = s.next();
					switch(option){
						case "1":
							Card temp = deck.deal();
							System.out.println(temp.suit + temp.face);
							players.get(i).handle(temp);
							if(deck.cards.size() == 0){
								deck.reset();
								for(int k = 0; k < players.size(); k++){
									for(int t = 0; t < players.get(k).cards.size(); t++){
					...
					players.get(i).points();
					if(!exit && players.get(i).count <= 21){
						i--;
					}
					else if(players.get(i).count > 21){
